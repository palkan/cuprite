...
class Cuprite {
    ...
    if (this.isVisible(node)) {
      if (node.nodeName == "TEXTAREA") {
        ...
        if (node instanceof SVGElement) {
          ...
          return this.normalizeWhiteSpaces(this.extractVisibleText(node));
        }
      }
    }
  }
  ...
  isHiddenByOverflow(node, parent) {
    const nodeRect = node.getBoundingClientRect();
    const parentRect = parent.getBoundingClientRect();

    return (
      nodeRect.top > parentRect.bottom ||
      nodeRect.bottom < parentRect.top ||
      nodeRect.left > parentRect.right ||
      nodeRect.right < parentRect.left
    );
  }

  // Extract visible text from the node taking into account elements that are not visible
  // due to overflow:hidden
  extractVisibleText(node, overflowParent) {
    ...
    // Find nearest ancestor that has overflow:hidden
    if (!overflowParent) {
    ...
    if (overflowParent && this.isHiddenByOverflow(node, overflowParent)) {
      return "";
    }
    ...
    return Array.from(node.childNodes).reduce(
      (text, childNode) => text + this.extractVisibleText(childNode, overflowParent),
      ""
    );
  }

  normalizeWhiteSpaces(str) {
    const replacedStr = str.replace(/(\s|\n)+/g, (match, p1, offset, string) => {
      if (offset + match.length === string.length) {
        // Last consecutive line break
        return '\n';
      } else {
        return ' ';
      }
